...
var AthleteDetails = Backbone.View.extend({
  bindings: {
    "#first_name": { observe: "first_name" },
    "#last_name": { observe: "last_name" }
  },
...
var RaceRow = Backbone.View.extend({
  tagName: "tr",
  events: { "click .delete": "deleteRace" },
  bindings: {
  ...
  render: function() {
    ...
    Backbone.Validation.bind(this, {
      valid: function(view, attr, selector) {
        ...
        view
          .element(attr + "-" + view.model.get("id"), selector)
          .parent()
...
var ClaimView = Backbone.View.extend({
  el: "#container",
  events: { "click #add": "addNewRace", "click #submit": "submitForm" },
  render: function() {
    ...
    if (this.model.isNewAthlete()) {
      ...
      var race = new AthleteRace({ id: this.createId() });
      ...
      this.model.fetch({ success: this.renderAthlete });
    }
  },
  renderAthlete: function() {
    var details = new AthleteDetails({ model: this.model });

    var ecap = new Ecap({ model: this.model });
  ...
  renderRace: function(race) {
    var raceRow = new RaceRow({ model: race });
